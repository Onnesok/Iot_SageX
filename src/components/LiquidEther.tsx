  ...
  useEffect(() => {
    ...
    const color_frag = `
      ...
      void main(){
        ...
        lenv = pow(lenv * 1.5, 0.7);
        lenv = clamp(lenv, 0.0, 1.0);
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      init() {
        if (!Common.renderer) {
          console.error('LiquidEther: Common.renderer is null');
          return;
        }
        if (!this.props.$wrapper) {
          console.error('LiquidEther: $wrapper is null');
          return;
        }
        ...
        // Ensure wrapper has dimensions - use viewport as fallback
        if (wrapper.clientWidth === 0 || wrapper.clientHeight === 0) {
          console.warn('LiquidEther: Wrapper has zero dimensions, using viewport size');
          // Force a resize with window dimensions
          Common.width = Math.max(1, Math.floor(window.innerWidth));
          Common.height = Math.max(1, Math.floor(window.innerHeight));
          Common.renderer.setSize(Common.width, Common.height, false);
        }
        ...
        // Remove canvas if already exists, then add it
        ...
        // Ensure resize is called after Output is created
        this.resize();
        ...
        console.log('LiquidEther initialized', {
          ...
          rendererWidth: Common.width,
          rendererHeight: Common.height
        });
      }
    ...
    // Start immediately since container is always visible (fixed background)
    ...
    if (webglRef.current && !document.hidden) {
      webglRef.current.start();
    }
